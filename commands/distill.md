---
description: 沉淀定稿——把复盘与【待蒸馏】队列经质量闸裁决后落盘 memory / 坑卡（用户触发或 AI 发起——闸内仍人工逐组裁决，支持 --scope）
---

# /crules-flutter:distill [--scope <需求/上线批次>]

把稳定下来的知识从「随手层」定稿进「制度层」。**触发时机由需求方判定**：上线 / 交付 / bug 修复后。
> 发起语义（1.0.2 显式化）：AI 可在交付汇报的「沉淀候选提示」处发起本流程（依赖此能力，故**故意不设** `disable-model-invocation`）；但闸内裁决（§5 三操作）仍逐组人工——发起 ≠ 落盘。
> **落点锚定**：本命令全部读写（memory 各库、`.review-ledger`、Gate 例外台账）锚定**宪法安装层**——工程根 CLAUDE.md 同层的 `.claude/memory/`，与会话 cwd 解耦；cwd 在子目录时先上溯再落盘，禁止就地新建 memory（防多库分叉——「聚合恒零」假信号之源）。
...
- **首跑空态**（`docs/复盘-*.md` 零文件且【待蒸馏】零命中）：**不静默空转**——汇报显式输出「尚无复盘文件，本次无可蒸馏候选」，并按 `memory/MAINTENANCE.md`「沉淀直写与走闸」的复盘文件模板当场提议新建 `docs/复盘-<日期>-<scope>.md`（**需求方点头才落盘**，模板格式以其为权威）——首次使用即把沉淀层的入口立起来
...
- **review 裁决聚合**（`.claude/memory/.review-ledger` 存在才执行；**不存在或有效行为零 → 不静默跳过**，汇报输出显式异常行「距上次 distill 零新增 ledger 行——度量空转信号（止损线观察点：2026-12-31 前累计仍未跨 30 则砍四数、只留 ledger 原始账——原条件「仍零样本」自首批样本落账起按字面永不触发，1.0.31 修正）」）：读全量 JSONL——字段 `date / agent / fid / sev / level / verified / verdict / note`（verdict ∈ 采纳 · 改写采纳 · 误报 · 撤条 · 搁置；`fid` 仅单报告内唯一、跨会话不唯一——勿按 fid 追踪裁决变迁；`note` 禁英文引号与换行——引号坏 JSON 行，中文引号「」表述）；**缺字段或坏行一律跳过并计数报告**，`搁置` 不计任何分母。**0 < N < 30 为悬空态：列明细 + 显式进度行「样本 N/30 · 比率按规则不输出」**（比率不输出的既定决策不变——防小样本 Goodhart 化；**禁把悬空态渲染成「无」**：「无」与零样本同形不可区分，观测带「仍无数」被外读成零样本即此成因，1.0.31 显式化）；N ≥ 30 输出四数——总发现数 / 采纳率（采纳+改写采纳）/ 误报率（误报+撤条）/ **按验证状态分桶的误报率**（`verified=推断` 被证伪比例——指导 reviewer.md 审查纪律修订的真信号）。异常模式（如推断级误报率 >50%）作提名进 §5 闸；聚合输出进汇报，**不常驻任何文件**
