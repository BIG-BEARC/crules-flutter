...
- **假设显式化**：非平凡实现开工前列出关键假设并标注**已核实 / 推断**，推断被证伪立即停下重对齐——不硬拗继续（承 v77 先例）
- **修一坑查同类**：修掉一个坑后检索同类模式，**限本任务范围内**——范围外只报告
- **清理自身孤儿**：自己的改动使 import / 变量 / 函数失效时同批清理；既有预存死代码只报告不动（承 v77 先例）
...
- **更简方案推回**：发现比既定方案更简的可行路径（含「无需改动」）时提出并给理由，采纳归需求方裁决——提出 ≠ 擅自改（承 v77 先例）
