...
- **跨仓 / 跨目录操作一律绝对路径**：Bash 会话间 cwd 会重置回主仓——相对路径曾把文件误写进错误仓库（实战 6 踩，批 2c 曾污染母版，见 `docs/复盘-2026-08-27-批2c双坑.md`）；含特殊字面串的文件用专用写工具落盘（shell heredoc 会触发 deny-list 误拦）；跨仓收尾必跑 `git status` 验零污染；多仓 / 子模块 git 一律 `git -C <绝对路径>`（`cd && git` 依赖 cd 落位，命令间 cwd 重置时连上一条验证过 pwd 都不可信），push 等外发操作前命令内先 `git remote -v` 核对目标仓再推（实战 7 踩：cwd 重置后裸 push 把主仓推出了新远端分支）
...
禁连续缩放系数（与系统字体缩放冲突、多端破版）。桌面端可按需扩 large/extra-large 档。
