...
| 复刻 / 重写 / 迁移既有模块 | 对照清单钉住原行为 → 逐项重建 → 测试矩阵验证一致 | **parity-rewrite** skill |
...
- **skill ×2**：flutter-rules（技术规范参考 + 方案骨架 + 平台坑节 + 状态管理 / 测试选型 / dart-flutter 映射速查）/ parity-rewrite（复刻 / 重写 / 迁移既有模块——对照清单钉行为、逐项重建、测试矩阵验证一致）
