...
> **fork 自 crules v74-fork-base 的审查纪律并合并 Flutter 专项**——本文件自持（不依赖 crules 包）：通用 10 条·编号 0–9（0–8 fork 自 crules 审查纪律，9 资损红线为 0.4.0 新增）+ Flutter 专项。完整审查方法论（业务可达性 Gate / L0-L2 / 资损克制 / 两阶段审查）见 [`进阶/审查与复核纪律.md`](进阶/审查与复核纪律.md)。
>
> 启用条件：Flutter 项目（app / plugin）做 code review 或工程化设计时。
> 实证口径：条目中引用的私有仓 commit 哈希（saas-cashier 等）属**生产实证但外界不可复现**——对外视作信任声明，规则本体（判定线/口径）不依赖具体哈希。
...
**4. 边界处理**：Loading 有反馈；Error 有用户可见提示；Empty 有占位；网络异常有兜底（离线 / 重试）；输入校验走统一工具；长操作（外设通信 / 上传 / 长连接）有超时与断连处理；**吞异常禁令**——`catch (_) {}` / catch 无日志 / unawaited Future / errorBuilder 返回空不记录，处置三态（处理 / 上抛 / 记录后降级）禁止第四态（吞）；边界场景清单——null / async 时序（取消 / 竞态）/ 生命周期（dispose 后回调）/ 弱网离线 / 超长列表 / 权限拒绝 / 多语言长度 / 平台矩阵各端 / 窄屏极值（分栏布局按最小窗口逻辑宽过一遍）；调试期空值先显式暴露或打日志，**不加兜底文案**——兜底把字段错位 bug 变成「显示为其他」这类更迷惑的现象，延迟定位（字段契约确认后再兜）。

**5. 国际化**：用户可见文本走 i18n；新增 Key 同步多语言文件；必要时执行 i18n 代码生成。

**6. 代码规范**：文件头注释（如项目要求）；无调试日志（**`avoid_print` lint 已拦**，抽查即可）；无硬编码 URL / 密钥；**服务端协议码 / 枚举值禁以字面量落 switch / 比较——必经枚举 fromCode 或常量类归口，发现 1 处必 grep 同类全量清点（单点修复 = 漏报；对齐在线版同款字面量照抄亦不放行）——实证：时间类型码 '1'/'2'/'3' 一类散落 notifier 十余处后收敛常量类，需求方定调「魔法数或硬编码应抽取出去」，高频重犯类（2026-09-21）**；UI 间距 / 尺寸类字面量归条 2 不在此重复；命名规范；**无新引入 lint 警告（`flutter analyze` 零 warning 是硬门——模板自带 `analysis_options.yaml` 基线）**；import 分组有序；资损线（支付 / 订单 / 打印 / 同步）结构化日志带追溯上下文——orderId 可记，**token / 密钥 / 完整卡号永不入日志**；机械项（analyzer / lint 违规）归 CI 基线，不占人审注意力；**工程自定义质量脚本（format / analyze / check）即唯一入口**——CLAUDE.md / §十二 写明并禁裸跑默认参数命令（实证：裸 `dart format` 一次波及 166 文件——私有仓生产实证，口径见头部）。

**7. 功能完整性**：spec 功能点均已实现（非 stub）；用户可完成完整流程；关键路径无崩溃；新增入口 / 路由已注册；老行为变更清单（方案 §1）外的行为变化 = 回归 bug。

**8. Git 规范**：commit message 清晰；不提交不该入库的生成文件；不提交调试临时代码；iOS 生成文件（Generated.xcconfig / flutter_export_environment.sh）按机器路径假变更，pathspec 排除后再提交；多仓 / 子模块 git 一律 `git -C <绝对路径>`，push 等外发操作前命令内先 `git remote -v` 核对目标仓再推（实证：cwd 命令间重置，裸 push 把主仓推出了新远端分支——私有仓生产实证，口径见头部）。

**9. 资损红线【0.4.0 新增】**：金额计算（精度 / 舍入）、幂等（重复提交 / 重试——**支付/退款/撤销/核销类接口核「请求号+在途锁」与重试白名单，判定线与口径见 flutter-rules skill 支付域**）、状态机非法迁移、并发扣减——涉钱 diff 强制逐项过；**修一处清点同类入口**——涉钱链路改动先检索同类提交 / 撤销入口全落点逐个核在途锁，禁靠记忆清点（实证：会员管理逐入口补防重漏了撤销入口，2026-09-20）；金额展示铁律——主金额固有宽完整显示不可截断（ellipsis 只作用于划线单价等辅助信息）。**存量数据兼容（条件条——涉本地存储结构 / SP Key / 缓存格式变更时查，其余跳过）**：迁移策略已裁决（迁移 / 双读 / 弃旧三选一）、降级路径明示（迁移失败读得出默认态，不白屏不挂死）——实证：SP 文件被旧版本改写致 deviceId 漂移「换号」（skill 坑库 SP 卡，saas-cashier 三笔补丁）。
...
- [ ] 响应式适配按项目 §七 所选方案对号审查（A1 查断点与禁缩放系数；A2 查 .sp 与系统字体缩放协调、平板场景排除）
- [ ] 新增 TextStyle 走统一注入点（平台字体一致性），无内联绕过
- [ ] 打包字体覆盖目标机型（OEM / 桌面）且 fontFamilyFallback 已声明
- [ ] 列表项 key 绑业务 id（spuId / orderId），不绑下标——防 element 复用致库存 / 选中态串行显示（涉列表增删重排时查）
...
### 无障碍（Flutter 专项，2026-09-08 补——审查侧对齐 skill A11Y 覆盖）
...
- [ ] 复刻在线版接口消费时追到 **logic / 消费端**确认字段是服务端返回还是**前端加工**（entity 有字段 ≠ 接口会返回；entity 头部样例响应注释优先看）——实证：payName 前端补写（私有仓生产实证，口径见头部）
- [ ] 对齐在线版只照抄**视觉与行为**，语义参数（wire 层字段映射 / 分页参数等）须过本项目不变量后再抄——实证：在线分页 offset 传非 0 即「跳过前 N 条」的脏语义被照抄，会员券八张空白回归（2026-09-20）
...
| UI / 视觉 | 页面骨架；复用组件；`ThemeExtension` 归口；暗黑 / 按所选方案适配 |
