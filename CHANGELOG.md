...
- **消费面文案（宪法 §一 机械执法）**：README「环境要求」节 2839 字单段黑话（批F2/销案/F10①/D 系引用）→ 430 字大白话（环境/双实现/已知边界/兜底四件事，编年史指针移 CHANGELOG 1.0.17–1.0.25）；随批清 init.md「外审 #5」、distill.md「评审 N5」两处内部引注
- **首跑断点三处补平**：① `/distill` 首跑空态——§2 队列清点增「零复盘+零候选 → 显式异常行 + 按 MAINTENANCE 模板当场提议新建 `docs/复盘-<日期>-<scope>.md`（点头才落盘）」，MAINTENANCE「沉淀直写与走闸」旁增复盘模板行（不新设权威）；② install.sh 骨架目录兜底 `mkdir -p indexes/ decisions/`（功能从未坏——宿主写入自动建父目录；坏的是首用者核对清单把「暂不存在」读成死引用；dry-run 不建守只读承诺）；③ reference-map 示例「见 git 历史首版」消费工程无你 git 可查 → 首版三行示例就地化为内联形样
- **分发口径回填（换代遗留）**：进阶 5 篇头注 7 处「复制本篇/复制 agents/ 到 .claude/agents/」与 install 全量落位、plugin 自动挂载正面冲突 → 改「已随 init 落位/plugin 挂载」口径（含 Agent编排「通用角色」部署位行、方案评审闭环轻量 tier 句）；help.md 同源声明死指针 → README「场景 → 入口」、五条第 5 条「5 分钟上手路径」死指针 → `进阶/上手教程.md` 实路径
- **最小概念五条去营销**：标题「先装进脑子的全部」删「全部」二字（实际概念面 11 项不止五条）+ 尾行「其余概念不必预习、遇到 AI 会主动走位、查概念地图即可」——审计「『最小概念』是营销口径」成立的零成本实质修正
- **test-self 断言 +2（48→50）**：① 消费面黑话禁令闸（分发 md 禁 批X/F#/D#/销案/金丝雀/撞号/外审#N/裁决单 族代号；README 维护节起截断=维护者面放行；HTML 注释豁免——生成器围栏注释含 canonical 属既有形态）② 文件锚定散文指针闸（「X『Y』节」的 Y 须真是 X 的标题——1.0.32 链接闸对散文形态的补全；「来源/出处/拆出」溯源句豁免——platform-pitfalls 指向拆分前旧标题属合法历史）；两闸均过负控：注入坏形态被咬、豁免面与真链不误伤
- **验证**：test-self **50/0**（Windows 实机 Git Bash；两闸首跑红 1=溯源句假阳性→豁免修正后绿，红得其所）；负控三样本（黑话注入咬/维护节不咬/死指针注入咬+真链活）；install 实跑临时项目两目录建成、--dry-run 零落盘；grep 残留「复制本篇/「的全部」/git 历史首版」零命中；跨会话共文件四件（README/test-self/install/MAINTENANCE）与遵守度批 4d79e3d 区段逐行互斥核对后各自入库
- **诚实边界**：distill 空态引导是命令指令文本（AI 行为层，无机械闸可锁——与 1.0.31 悬空态显式化同机制同信任级）；「语义级同步」类同源声明（help↔README）仍无逐字闸，本批只保证指针目标存在；两轴档位设计不动（2026-09-14 裁决既定）
- **观测带数**：① 常驻基线 app 16571/18000、plugin 15270/16500 字符（预算闸内；MAINTENANCE +1 行微增）② distill 四数：消费工程样本 8/30 · 比率按规则不输出（悬空态显式，同 1.0.31–1.0.37 口径）
- 双 json 1.0.38 + README 横幅同步（1.0.36 跳号见上注）
...
- **deny-list.py 两处**：① RM_SIG 边界类补 `&`（bash `&` 只是结束前一条命令、rm 照跑；`;rm` `|rm` 系分段先行无需入类）；② 入口段形状判据显式化——顶层非对象 / tool_input 非对象 / command 非字符串一律当「没送命令」放行（对齐既有 F10① 哲学：宿主永远送正规 JSON，畸形形状属探活族，弹窗打扰比重放行糟；顺带消除 str.get 崩溃路径）。双源同判收口：`&rm` 双 deny、畸形形状双 allow
- **夹具 + 契约锁**：`true &rm` 入 deny 夹具（both——ps 侧本就拦，钉住对齐）；py 驱动形状契约锁 1 例（断 allow 零输出，与「空 stdin 断 ask」同文件并存——防「一并收口」过度修正）；ps 驱动黑盒探针 ×6→×7 同款
- **agents 孪生入生成闸**：backend/frontend 共用「例外 + 判定线」句收进 `canonical/aggregate-exception.md`（第 5 块），两卡加围栏、scan_targets 扩 agents/、锚串 +2（backend 第 7 例：`底屽` 错字级漂移即此块脱管的实证）；围栏外各卡前缀语句保留手写
- **test-self 断言 +2（45→47）**：① hooks.json 结构看守——matcher 集 / 引用整体路径（CLAUDE_PLUGIN_ROOT + hooks/ 目录段）/ 兜底链 `||` / async 位期望值（deny 双 hook 必须同步、pending-updates 必须 async、Stop 同步——翻任一即红；首版 handlers 推导变量名遮蔽踩坑已修并注记）；② 对照表符号闸——`# py <符号> ↔` 行的符号须真实存在于 deny-list.py、禁行号形锚点回写、11 个关键符号缺一即红（行号历史漂过 ~37 行 / 11 行两次，改符号名后无漂可漂）
- **对照表重写**：全部行号锚改符号名 + 表头注防回写；ps1 内文唯一行号引用（L122）同步改符号指法；1.0.35 双源同判两处单列注记（不占 D-x 名额——非有意差异，系已修缺陷）
- **随批计数同步**：README 同文块说明四块→五块、render-blocks 头注、test-self 环境守卫行（其余 43→45）、ps 驱动「×138 例」→「百例级」（数字写死即下次漂移之源）
- **验证**：py 驱动失败 0（含新形状锁）+ `--diff` 170 例两路并跑不同判 0（闸本体已改，按 harness 纪律重跑）+ ps 驱动失败 0（82 拦 + 38 放 + 17 warn + 24 变异 + 探针 7，os=win 全跑）+ test-self **47/0**（Windows 实机 Git Bash）；五处修复全部先红后绿留痕：`&rm` 例红于旧 py（allow）、形状锁红于旧 py（ask）、同文块闸红于新块未登记、两条新断言首跑各红 1（Stop handler 遮蔽 / L122 残留——红得其所即断言有效）
- **观测带数**：①常驻基线零变化（agents 卡为按需派发面、canonical 块不入常驻 token；test-self 字符口径不变）②distill 四数：消费工程样本 8/30 · 比率按规则不输出（悬空态显式）
- 双 json 1.0.35 + README 横幅同步

## 1.0.34 · 分发工程批——验证收敛到必达点 + 坏值不流转 + 落位幂等一把伞

> 依据链：分发工程外部评审（2026-09-22）逐条对账——13 条成立 / 2 条修正（「read 在 EOF 撞 set -u 崩」实测不成立：EOF 落空串走保守取消，真症状是**无人值守静默取消**；「py_compile 缺 3」实为缺 4）。归类后根因三个：①同一验证清单多处复制、靠人手同步（release 验证 vs CI vs test-self 三套）②出错兜底继续走（`|| VER="unknown"` 让垃圾值流进写戳→戳守卫认不出→升级永久锁死「老项目」；2026-09-22 本机探针再次命中同形态）③落位状态机缺口（AO 分支绕开幂等——**全新项目双跑即实测红**：首装写基线、二次被判「自定义」翻 SIDE-CAR 多出重复伴生）。处置哲学与 F3 一致：能删清单就删清单、坏值诞生处即停、一把伞断言盖未来文件。

- **release.sh 验证收敛**：bump 分支删手工枚举清单（py_compile 3/7 漂移实锤）——写双 json → **读回断言**（marketplace 侧原只写不验）→ 跑全套 test-self（三 fixture 子进程实跑 6 个 py + 生成闸实跑 render-blocks，7 个 py 全被真实执行，强于编译枚举）；**tag 分支加双闸**：工作树必须干净 + test-self 必须绿才准打 tag——tag 是消费者实际拿到的工件，执法点从 bump 移到 tag（堵 bump→commit→tag 之间偷改）。发版路径与 CI 自此跑同一个东西
- **install.sh 取版本去 python 化 + 读不到即停**：`grep` 直读（版本号纯 ASCII 码页无关），删 `|| VER="unknown"` 兜底——无 python / 码页 / 进程首启失败三态统一「中止不写垃圾戳」；check-imports.sh 同款（原 `?` 兜底伪装「未检出戳」假绿）+ 版本差改**三态报方向**（一致/可升/源更旧，源更旧时不再劝「重跑 install.sh」）；deny-list.py 头注 py_compile 指针随迁
- **install.sh 状态机补全**：①未知参数即红（`*)` 分支 exit 2——原 `--forc` 手误=静默普通安装）②`--yes` 无人值守升级正门 + `read` 兜 `|| true`（EOF/关闭 stdin 均落保守取消，AI 驱动不再靠喂管道）③**降级默认拒**——戳版本 vs 源版本 sort -V 比方向，`--allow-downgrade` 显式放行（普通安装与 --upgrade 双入口都拦）④AO 落位幂等三修：已是基线→SKIP（堵二次运行翻分支）；SIDE-CAR 伴生走三态（缺失才写 / 已在 SKIP 保用户合并成果 / --force 出 .new）；UPGRADE 分支同 SKIP 前置
- **test-self 断言 37→45（+8）**：未知参数红 / 版本读不到中止（判据三条件防假红：rc≠0+提示语+零落盘）/ --yes 执行 / 降级拒+放行 / **双跑不变一把伞**（同目标连跑两次 install，diff -r 零增量——自动覆盖未来新增落位文件的幂等，替代逐文件断言）/ 伴生哨兵（重拷即红）/ 双 json 相等 + release 闸防删改锚 / check-imports 降级方向；环境守卫注释计数同步修正（1.0.33 起漏更，一并修）
- **README 升级节**：--yes / --allow-downgrade 两行说明
- **验证**：test-self **45/0** 实跑绿（Windows 实机 Git Bash）；新断言全部先红后绿（落地前实测 37/8——8 条新断言逐条红于对应缺陷，双跑断言的 diff 输出即前述 AO 实测增量）；release.sh bump 分支自跑全套通过即证收敛生效
- **观测带数**：①常驻基线零变化（本包分发面脚本，非宪法模板；test-self 字符口径 app 16571 / plugin 15270 不变，/context token 快照欠账同 1.0.32 口径）②distill 四数：消费工程样本 8/30 · 比率按规则不输出（悬空态显式）
- 双 json 1.0.34 + README 横幅同步

## 1.0.33 · 三 hook 进程期 AV 弹框压制（Windows）——注入型管控 agent 崩溃框不再挂起 hook

> 依据链：2026-09-22 本机 A/B 五组实测（屏幕窗口快照差分法；合成 = NULL 页写入真访问违例）——python 脚本内 `SetErrorMode(0x2)` 后崩溃：**无框、静默死、rc=3221225477(0xC0000005)**；不设则框必现（与本机真框同款文式）。**父进程预设法无效**：CPython 启动把继承的 error mode 覆写为 1（父设 3 → 子实读 1），守卫必须落在脚本内部；起效位仅 0x2（单设 0x1 框照弹）。hook 是高频 spawn 面（本机每次 Bash 调用 = 3 个注入进程），进程期 AV 弹模态框 = hook 挂起等点击直至超时——1.0.22 已保证崩溃不走闸（非零退出走 `||` 兜底 / ask），本批只压 UI，判定行为零变化。**方法论教训随批记录**：硬错误框的窗口 pid 恒为弹框宿主而非崩溃进程、且框可存活于崩溃进程死后——首版实验因旧框未收走把后续组全误判（正解：前置清屏基线 + 只认新出现 hwnd 差分 + 起跑门 + 框出现即 WM_CLOSE 收走）。

- **hooks/deny-list.py + pending-updates.py + stop-reminder.py**：import 后各加同款守卫——win32 判定 + `ctypes.windll.kernel32.SetErrorMode(0x0002)` + try/except 全包（压不成框仅退回原状；POSIX 无此调用路径零影响；注释带「三 hook 同款同改」同源指针与实测依据五要点）
- **deny-list.ps1 不加**（诚实边界）：PowerShell 侧设 error mode 须 Add-Type P/Invoke、每进程重编译代价不可接受，且实机 AV 框观测均在 python 侧
- **启动期 0xc0000142 族不在此覆盖**（发生在用户代码之前）——该族由机侧关框器负责（工具留档 `%LOCALAPPDATA%\crules-diag`）
- **test-self 断言 36→37**：三 hook 守卫防删改闸（win32 判定 + SetErrorMode 双串在位，缺一即红）
- **验证**：test-self **37/0** 实跑绿（fixture 实际执行三 hook，守卫真实跑到、无副作用）；A/B 证据链见依据链
- **观测带数**：①常驻 token 面零变化（hook 是调用期执行面，非上下文常驻）；②distill 四数：样本 8/30 · 比率按规则不输出（同 1.0.31/1.0.32 口径）
- 双 json 1.0.33 + README 横幅同步

## 1.0.32 · 信息架构批——指针可解析 + 坑库症状速查 + 三查闸——外部信息架构评审核实落地

> 依据链：外部信息架构评审（2026-09-22 交付）三弱项逐条核实——弱1「跨文件指针不可解析」半对（skill 内 checklist 裸写属实、可 Glob 达但形式不一致；C 号指针带版本锚但 CHANGELOG 不随装分发属真）；弱2「缺症状→卡反查索引」属实（16 卡 149 行 17.1k 字符，症状全埋卡体，检索粒度止于「场景→文件」）；弱3「加载时机四处、记忆库体系清单实质漂移」精确对（漏 reference-map / platform-pitfalls 实锤——后者恰是引平台依赖必读那份；「NAVIGATION 15 条死链永不创建」定性错：按需创建的占位，但占位用链接格式确属不当）。核实另发现同族漂移两处：README 基线复测快照滞留 1.0.7 实测数、坑库头注「首批 ≤10 条」对 16 卡。处置原则 = F3 既有哲学（防漂移从检测前移到构造）逐条归位：删复制 / 上机械闸 / 去会过期的句式；机器管不了的三件（坑卡内容时效、活的 /context 测量、关键词语义覆盖）如实留义务。

- **坑库症状速查表 + 卡「关键词」行**（skill platform-pitfalls.md）：16 卡各加首行「关键词」（症状反查词、一卡多说法）；文件头新增「症状速查」表 = 全部卡（关键词 / 平台 / 卡题）的机械汇总——检索粒度从「场景→文件」细化到「症状→卡」。表与卡由 test-self **逐字对账**（改卡不同步表 / 加卡漏补 / 手改表即红）；头注「首批 ≤10 条」过期数量句删除（宁缺毋滥口径保留）
- **NAVIGATION 占位去链接化**：15 条 `indexes/*` + 1 条 `decisions/` 占位由链接格式改纯文字——占位指向按需创建的生成物，不是可点链接；占位注释同步写明（链接须指向已存在文件 = 新链接闸口径）
- **记忆库体系.md 删清单复制块**：「会话开始时加载」节曾抄 5 漏 2——整块删除改指 memory/README.md 总表（该文件自己的单一权威宣言自此兑现，1.0.4「进阶侧表已删改指针」注释随之成真）；test-self 上**回潮闸**（进阶/ 代码围栏内罗列 ≥3 个 memory 文件名即红）
- **skill 指针限定**：references 内 6 处裸「checklist」→「项目根 `checklist.md`」（skill 随 plugin 缓存分发，与消费工程根不同目录，裸写不可解析）；坑卡 2 处「见 CHANGELOG 0.6.0 条」→「见 plugin 仓根 `CHANGELOG.md` 0.6.0 条」（消命名空间歧义；C 号保留——出处链不删）；test-self 上**限定闸**
- **README 基线复测去快照化**：「最新：新装 5.5k〔1.0.7 实测…〕」快照句删除——快照句必过期（1.0.7 的数挂到 1.0.32 才被发现即实证）；实测数随 minor CHANGELOG 条落（与观测带数①同一落点），本节只留义务与口径
- **test-self 断言 31→36（三查闸 + 两对账）**：①链接解析闸——全仓 md 链接须指向存在文件（CHANGELOG 豁免：历史流水账链到后来删除的文档属正常）；上闸前预扫实锤 **17 条死链全数清掉**（NAVIGATION 16 占位 + 工程化流程.md 交付汇报模板 1 示例占位）。②坑库速查逐字对账。③memory 清单回潮闸。④checklist 指针限定闸。⑤checklist 编号 0–9 对账（自述与实际一致）
- **验证**：test-self **36/0** 实跑绿（Windows 实机 Git Bash + Python 3.14）；速查对账闸**反向验证三态**——删卡关键词行得红（指认到卡）、手改表得红（期望/实际逐行 diff）、原文件绿；常驻面净变化 **−158 字符**（NAVIGATION 去链接化省 > 注释增；app 16571 / plugin 15270，预算闸内）
- **观测带数**：①常驻基线：字符口径 app 16571 / plugin 15270（test-self 实测）；`/context` token 快照**本会话不可测（需新装会话），如实记欠账**——下次 minor 按新口径补测；②distill 四数：样本 8/30 · 比率按规则不输出（悬空态显式，同 1.0.31 口径）
- 双 json 1.0.32 + README 横幅同步

## 1.0.31 · distill 聚合悬空态显式化 + 止损线复活——「四数仍无数」两态同形与死条款收口

> 依据链：2026-09-22 本仓探针——按八字段写 18 行 ledger（1.0.30 三轮评审 + 主控发现回填）读回 18/18 有效、0 坏行、搁置不计分母正确生效（N=17），证明落账 / 读取 / 聚合管道通；但 0 < N < 30 在旧规则下无专属处置——只列明细、比率不输出，与零样本在观测带渲染同形（CHANGELOG 含「四数」行累计 27、「仍无数」19 处；09-22 外审把「仍无数」读成「零样本、闭环未转」即此成因）。另两处同源漂移随批核出：distill §7 止损锚滞留 1.0.9 的「2.0 前」未随 1.0.11 改时间触发（2026-12-31）；README 观测带四数定义滞留 1.0.3 旧式（裁决总数 / 推断数 / 驳回数）与 §7 现行四数（总发现数 / 采纳率 / 误报率 / 分桶误报率）不一致。

- **commands/distill.md §7 两处修正**：①悬空态（0 < N < 30）显式进度行「样本 N/30 · 比率按规则不输出」——比率不输出的既定决策（防小样本 Goodhart 化）不变，**禁把悬空态渲染成「无」**（「无」与零样本同形不可区分，即误读之源）；②止损条件「仍零样本」→「累计仍未跨 30」——原条件自 09-20 消费工程首批 8 行落账起按字面永不触发、已失效；锚同步 2026-12-31（与 README 时间触发对齐，消 1.0.11 漏同步）
- **README 观测带数行同步**：四数定义对齐 §7 现行式；`<30` 记数不计率补悬空态进度行语义；止损条件同步「累计仍未跨 30」
- **探针台账用完即删**：18 行验管道后清除本仓 `.claude/memory/.review-ledger`——狗粮不转正（本仓评本仓属自证，采纳率恒高无信息量）；distill 落点锚定禁令对消费仓一字不动
- **已知未修（如实记）**：①模板仓自身无宪法层 → 无合法 memory 落点（1.0.28 落点锚定为消费仓分叉病而写，本仓被字面扫中——留待狗粮转正决策时裁）；②消费仓在官方 worktree 内跑 distill 时锚点解析落空（CLAUDE.md 随 checkout 在、memory 因 gitignore 不在）——两处同型缺口，本批不扩权
- **验证**：纯规则文本批（无新执行面 / 无闸面，F11 口径同 1.0.28）；grep 一致性核——「仍零样本」活规则面 0 处（仅存 §7 修正注引文 1 处）、止损条件 distill / README 各 1 处同文、悬空态 distill 1 处、「2.0 前」旧锚非历史面 0 处；test-self 31/0 保持绿
- **观测带数**：①常驻基线零变化（纯规则文本批，无宪法 / 命令 / hooks 改动）；②按新规渲染——消费工程 ledger **样本 8/30 · 比率按规则不输出**（悬空态显式，不再写「无数」）
- 双 json 1.0.31 + README 横幅同步

## 1.0.30 · 协议码字面量禁令 + 消费工程坑卡上移 6 张 + 漂移队列按会话分文件——魔法数高频重犯与平台坑吸收 + 多会话队列互踩根治

> 依据链：消费工程（saas-cashier，2026-09-20/21）——①**魔法数 / 硬编码高频重犯**（需求方点名吸收）：服务端协议码字面量散落 switch / 比较，实测收敛一批（时间类型码 '1'/'2'/'3' 类十余处归口常量类）并在消费仓 checklist 落条（2026-09-21 需求方拍板，未提交；本仓同步吸收通用版）；②消费仓两处坑库 6 张通用卡满足 skill 侧「入预置门槛①我们实证踩过」，按坑库分工设计上移（TabBar 双卡与 InputDecorator / screenutil 卡为 2026-09-20 distill 裁决与探针实测；fvm 坏 dill 卡与 SDK 版本纪律卡为 2026-09-20 需求方拍板沉淀，后者原卡「三方依赖组合」误判已被复盘推翻后重写）。

- **checklist.md 第 6 条 +「协议码 / 枚举值字面量禁令」**：服务端协议码 / 枚举值禁以字面量落 switch / 比较，必经枚举 fromCode 或常量类归口；**发现 1 处必 grep 同类全量清点**（单点修复 = 漏报）；对齐在线版同款字面量照抄亦不放行；UI 间距 / 尺寸类字面量归第 2 条不重复
- **skill platform-pitfalls +6 卡**（TabBar 双卡、InputDecorator 基线制、screenutil .sp 时序入既有节；新增「工具链」节收 fvm 坏 dill 诊断卡、SDK 版本纪律卡）：全部带生产实证出处 + 最后核验；分发面 skill 触发体积 +约 60 行，常驻面零变化
- **验证**：test-self 29/0 实跑绿（随批前——本段为上一子批记录；随批后总数见下条）；纯知识 / 审查纪律批——不触闸面、无常驻面变化、无新执行面（F11 口径辨析，故无新断言）
- **观测带数**：常驻基线零变化；distill 四数仍无数（止损线 2026-12-31）
- 双 json 1.0.30 + README 横幅同步
- **hooks 漂移队列按会话分文件（1.0.30 未发版随批补——多会话共享工作树「队列互踩」根治，方案 D1–D8）**：写侧 PostToolUse 与读侧 Stop 均按 `session_id` 路由到 `.claude/memory/.pending-updates.<sid>`，并行会话各写各队列、收尾各清各的；无 sid 回退旧单文件（D2 兼容旧宿主）；读侧自己为空读 legacy（升级存量一次性迁移，D3）；他人队列文件 mtime>24h 计孤儿**只报不删**、新鲜不提（D4——删除留给人，防误删崩溃会话收尾线索）；git 快查已入队集并入所有队列文件行（D5 降共享树噪音）；sid 文件名净化 `[A-Za-z0-9_-]`（D6）。**R1 销案**：子代理 PostToolUse 的 session_id 归属官方文档未载明，嵌套无头会话探针实测（2026-09-21）＝主会话 id（agent_id 仅子代理上下文并存）——子代理编辑落主会话队列，方案成立。**D8 顺手修两个既有缺陷**（写 fixture 时暴露）：①会话 cd 进子目录后 cwd 漂移 → `.claude/memory` 落空 → 队列整段静默（写读两侧改自 cwd 向上逐级找 NAVIGATION.md 定根；读侧闸位与写侧对齐——无 NAVIGATION 即静默）；②`/.git/` 排除检查用正斜杠字面串、Windows 反斜杠路径恒不命中（归一后判，fixture 锁定）
- **验证（随批）**：写侧新 fixture `test_pending_updates.py` 12/12 + 读侧 `test_stop_reminder.py` 18/18（原静默四态保持锁定）实跑绿（Windows 实机 Python 3.14）；test-self 断言 29→31（+写侧 fixture 挂载 +gitignore 升级迁移）；MAINTENANCE 自检清单与 git 分层 / 记忆库体系 / 上手教程 / memory·README / help / README 文件清单同源面同步
- **review 三回合修六项（随批）**：**回合一**（reviewer 首审，无 🔴）①**R5 分隔符（实测实锤）**——写侧落盘 `os.path.relpath` 在 Windows 返回反斜杠，而 `git status --porcelain` 恒输出正斜杠（本机实测签名 `M skills/…`），不归一则读侧「已入队」比对在 Windows 恒不匹配、既有「已入队不重复报」（1.0.12 批B F1）与 D5 声称双双失效——写侧落盘归一、读侧 `norm_paths()` 归一（兼容 1.0.30 前反斜杠旧行）；②**R3 fail-open 收口**——合法 JSON 但字段异型（`tool_input` 非 dict / `file_path` 非字符串 / `session_id` 非字符串）时旧写法抛未捕获异常或产垃圾条目，改 isinstance 严格判定按「无此字段」退场；③**R6 文案指向**——D3 回退命中（条目来自 legacy）时文案曾让人清「本会话队列文件」（本就为空），改按来源指向旧单文件。**回合二复读**（8 项全消解、无 🔴）新发现：④**N1 顶层形状**——`json.load` 成功 ≠ 形状合法，顶层非对象（`[]` / `"x"` / `123`）时 `data.get` 抛 AttributeError 在 try 外即崩溃，与 fail-open 头注不符，两 hook 各加 `isinstance(data, dict)` 守卫；⑤**N2 旧行不自愈**——队列含 1.0.30 前反斜杠旧行时再次入队会追加正斜杠新行、同一文件留两行（去重按原始串），读旧行处一并归一。**注**：N2 初修时只归一了首次读取、漏了 flock 后为防竞态的重读（同含旧行），被新断言当场抓住后补齐——「一处语义两处执行」的典型，凡归一/过滤类改动须全数核对执行点；⑥N3 三处顶格打磨（CHANGELOG 时间线分层标注 / install.sh dry-run 计数注 / 注释与行为对齐）
- **gitignore entry 迁移（随批）**：队列行由精确名 `.claude/memory/.pending-updates` 改通配 `.claude/memory/.pending-updates*`（按会话分文件后同名多份），install.sh 升级时**删除旧精确行再追加通配行**——精确行比对不命中通配 entry，不迁移会留下双行近似重复（实测 5 行 → 迁移后 4 行，test-self 新断言锁；reviewer R2 所见为修复前快照）；README 文件清单行随之修正（「三行」实为四行、漏 gate 台账，1.0.7 起既有瑕疵）
- **覆盖差如实记**：D8 的 `/.git/` 归一修复（正斜杠字面串在 Windows 恒不命中）**CI 不可回归**——ubuntu 上 `os.sep=='/'` 新旧写法同结果，仅 Windows 实机区分（本机实测证据：归一前反斜杠路径落盘）；「向上定根」则跨平台可回归。勿以「CI 绿」误读为该项已锁

## 1.0.29 · 表征断言锁实测值 + 涉钱修一处清点同类 + 对齐不照抄语义——消费工程四缺陷复盘三条入库

> 依据链：消费工程（saas-cashier smart-pos）会员管理批四缺陷复盘（2026-09-20，需求方移交本仓收口；代码修复已在消费仓落地 cac5399c1）：M1 照抄在线版分页脏语义（offset 非 0 即「跳过前 N 条」）致券列表空白回归；M2 逐入口补防重漏了撤销入口（资损红线闸在、写时清点靠记忆）；M4 表征断言锁理想值 ≤0.5（InputDecorator 基线制定位结构性不可达）红着进仓。共性 = 清点靠记忆、断言靠愿望、对齐无兜底。规则面盘点：支付域三层口径、资损红线幂等条均在，缺写码时清点与断言取值两条纪律——本批补齐；另随记需求方当日明示的工程经验「所有资金类接口一律做防重」。

- **testing.md +「表征断言先实测再落锁」**（两表通用）：断言值取实测可达值——先跑拿数、查机制给界，不取理想值；锁下的值注明机制下限与出处；残差视觉可否接受属需求方裁决项
- **checklist.md 第 9 条 +「修一处清点同类入口」**：涉钱链路改动先检索同类提交 / 撤销入口全落点逐个核在途锁，禁靠记忆清点
- **checklist.md 复刻专项 +1 复选条「对齐不照抄语义」**：在线版视觉 / 行为可照抄，语义参数（wire 字段 / 分页映射）过本项目不变量后再抄
- **payment.md 溯源 +2026-09-20 撤销充值漏在途锁实证**，随记需求方工程经验（资金类接口一律防重）
- **版本号取 1.0.29**：1.0.28 留给工作区另一在制批（落点锚定，常驻面批次）防撞号
- **验证**：test-self 29/0 实跑绿；本批不触闸面、无常驻面变化；三条均为审查 / 写码纪律（非机制、无新执行面，故无新断言——F11 口径辨析）
- **观测带数**：常驻基线零变化；distill 四数仍无数（止损线 2026-12-31）
- 双 json 1.0.29 + README 横幅同步

## 1.0.28 · 记忆库落点锚定（一仓一 memory）+ 裁决必附具体内容——多库分叉与裸裁决两族防再犯

> 依据链：消费工程 saas-cashier 实证（2026-09-20）——一仓装了两套宪法（根 v1.0.25 + 子包 v1.0.7）各带「记忆库接线」指向自家 `.claude/memory/`，早间 distill 三张坑卡落子包库：「聚合恒零」类假信号的结构性根源是**库分叉**而非履职违规；另同日裁决摩擦暴露裸裁决请求——只说「请裁决」不附对象全文，需求方翻旧消息找裁决对象。

- **plugin/CLAUDE.md + app/CLAUDE.md「记忆库接线」节 +「落点锚定（一仓一 memory）」**：memory 全家（坑卡 / patterns / ledger / Gate 台账）读写锚定宪法层、与会话 cwd 解耦（子目录先上溯再落盘、禁就地新建）；**同仓禁装第二套宪法层 / 第二 memory**（monorepo 子包引根层，不重复安装）
- **commands/distill.md 头部 + 落点锚定行**：命令全部读写（memory 各库 / ledger / Gate 台账）锚定宪法安装层
- **两份宪法 CLAUDE.md §一红线 +「裁决必附具体内容」**：裁决对象就近同屏——可枚举走 AskUserQuestion（条目全文进问题与选项说明，超 4 条分批、每批自包含）；长列单 / 开放裁决就地列全文（编号 + 位置 + 原文 + 建议）；禁「请裁决」不附内容
- **验证**：纯规则文本批（常驻面每份宪法 +2 行、命令头 +1 行；无新执行面 / 无闸面——F11 口径辨析，无新断言）；grep 一致性核——落点锚定 plugin/app/distill 各 1 处、裁决必附 plugin/app 各 1 处
- 双 json 1.0.28 + README 横幅同步
