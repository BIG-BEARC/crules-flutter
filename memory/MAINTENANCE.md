...
> **孪生声明**：「触发即更新」表原与 `进阶/记忆库体系.md` 孪生双份；现已消解——**本表为单一权威**，进阶侧改为指针不复制（同 twin:mem-files 哲学：单一权威 + 机器看守）。

## 触发即更新（强制）

> 本表是「你做了什么 → 必须更新」的**单一权威**（进阶篇为指针，不另复制）。
...
- **复盘文件模板**（`docs/复盘-<日期>-<scope>.md`，直写层、零仪式）：头部日期 + 事件经过（踩了什么坑 / 怎么发现 / 怎么绕或修）+ 值得沉淀的段落按上条格式标【待蒸馏】。形状不限，但每条候选须带出处（file:line / 命令输出 / 需求方原话）——`/distill` 首跑无复盘时会当场按本模板提议新建
