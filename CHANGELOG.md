...
## 1.0.46 · MQTT 域沉淀 + 外设域三拆——新增 network-comms.md（15 卡）与 printer / scale / scanner 三域文件，iot-devices.md 枢纽化

> 依据链：需求方 2026-09-24 指令「mqtt 相关知识也需要整理出来」（硬件外设批1-3 期间反复以「归网络域」暂缓的素材域独立成批）；同日追加裁决「打印机/电子秤/扫码枪是不是拆分出来比较合适？」→ 三域全拆 + 「客显可以并到电子称当中」——拆分并入本版本批（顺延 bump 会使 1.0.46 成「有头无 tag 又非在建版本」真缺口触缺口闸；多批收口一个版本批循 D9 先例）。采证源 saas-cashier（日活 7w 餐饮 POS）纯只读（master-fix2108 线）；两路采证（引擎/连接面 + 业务主题/到达性案卷面）→ 我方三轮机械复验 → 需求方裁决 → 落盘验收。MQTT 细节见 [复盘-2026-09-24-MQTT域沉淀.md](docs/复盘-2026-09-24-MQTT域沉淀.md)，三拆细节见 [复盘-2026-09-24-外设域三拆.md](docs/复盘-2026-09-24-外设域三拆.md)。

- **新增 references/network-comms.md**（网络通信域，MQTT 主体，47 行 15 卡四节）：连接生命周期 6 卡（重连成功≠订阅还在·双路径补订链三断点·本域头号坑 / 库级自动重连禁用+手写定时器·指数退避引入又被注释·mInterval 五跳演化链 / 单例 clientId 互踢 / 登录登出切店耦合·前后台零分支 / 连接状态机多处持有+误红系列五笔 / 心跳超时参数自检表）；订阅与 QoS 3 卡（QoS 双轨+下行无对账·外卖新单无声案实证 / 分发面 EventBus 5 监听方+批量帧只取第一条 / 叫号未知 code 静默丢弃+误导日志）；业务到达性案卷 5 卡（推送到达但打印机未就绪 / 服务端重推重复打印·有界去重兜底 / 叫号制作中重复入列 / 大屏 TV 跨店串台·按店隔离 key 自引入日从未生效 / 优惠付款码播报寄宿 atMostOnce）；broker 配置安全 1 卡（凭据明文/1883 明文/证书校验全放行/日志打密码——机制层全脱敏）
- **三例现行缺陷**按需求方裁全落并标「现行缺陷，接入前自查」：TV 跨店串台（修复 2d11e59ef 在支线未并主线，merge-base 实证）/ 优惠付款码寄宿面 / 未知 code 丢帧
- **外设域三拆（同日追加裁决）**：`printer.md`（打印域 23 卡六节：蓝牙 5 / USB 4 / 网口 3 / 打印专项 7 / 内置 3 / 标签 1）、`scale.md`（秤·客显串口设备域 23 卡三节：串口底座 5 / 秤 11 / 客显 7——客显并秤系需求方裁，平行双实现 + 互斥同口内聚）、`scanner.md`（扫码域 7 卡两节：HID 4 / 盲扫与业务面 3）——卡面自 iot-devices.md **逐字迁移零内容变更**（仅节内互链指针改指新落点，迁移经行集机械比对证实）；iot-devices.md 220→93 行枢纽化（通用铁律 / 外设域文件地图 / Android 14 前置门 / 钱箱 / 副屏 / 语音播报 / KDS）；检索错位收口——文件原按「通信方式」分节而任务按「设备」来（修一个网口打印问题须翻六节），拆分后按设备一跳命中
- **指针面同步**（防死指针闸同族）：SKILL.md 索引新增网络通信域行；agents/backend.md 新增「网络通信（MQTT / 消息推送）」节指针；iot-devices.md 三处「归网络域」表述改指新文件（网口节 MQTT 失订指针、语音播报节节导与节尾，随三拆落 printer.md）。三拆指针面五处同步：SKILL.md 外设行 1→4 行（打印 / 秤客显 / 扫码 / 枢纽各一行）、agents/backend.md 设备通信节、platform-pitfalls.md Android 14 卡出处行设备指针、network-comms.md 节尾外设指针、build-release.md 混淆节秤锚落点
- **验证**：两路采证哈希并集 73 笔正则抽取 cat-file 零幻影、merge-base 在线性逐笔判定 ON 70 / NOT-ON 3（恰为支线锚 2d11e59ef / d0f003d5c / 173e731c8）；覆盖面 16 项计数全等（mqtt 133/推送 45/重连 34/…）；diff 逐字与现值回读吻合；落盘面 T6 锚 43/43 cat-file 零 BAD、T2 占位 0、V5 脱敏全词（凭据/实例名/域名/门店号/订单号/cashierId 值/人名/工单号/内部链接）23 词全零、test-self PASS=61 FAIL=0；三拆落盘后全套闸复跑 PASS=61 FAIL=0（卡数守恒 74=23+23+7+21、行集比对卡体零丢失，见拆分复盘）
- **诚实边界**：①采证报告 3 处瑕疵经复验纠偏（printIdQueue 现行落点系 printer_manager.dart，报告所指 printer_service.dart 在采证线已不存在；外卖播报码表现值九码非四码；print 失订窗口时间线按正确口径落卡），另 mInterval 演化链两路报告各见一半、合并补全——「旧路径当现行路径」登记为新亚类；②外卖无声案客户端定位完成、待服务端协查（案卷口径如实转登）；③三例现行缺陷中 TV 串台修复在支线、favorpay 寄宿与未知码无修复锚；④data[0] 丢尾无实证事故，观察项不单独立卡；⑤cleanSession 未在码面设置，库默认不猜不断言
- **观测带数**：references/ 8→12 文件（+network-comms.md / printer.md / scale.md / scanner.md）；iot-devices.md 220→92 行（三拆枢纽化）；network-comms.md 47 行 15 卡；采证覆盖 `--grep mqtt -i` 133 笔两轮全过目
...
- **iot-devices.md 批3 落盘**：客显 8 卡入「副屏 / 数字客显」节（枚举/构造异常链先于 open、关开关清零竞态、开关策略三连反转终态保持连接、LED 字库+isValidPrice 正则缺陷、写失败无界递归反模式、流控六参数如实标推断、SP 释放+秤互斥）；新增「语音播报」节 9 卡（触发源生命周期绑页面双面坑、音频焦点抢占队列停摆、win7 引擎路线九笔摇摆、白名单多份维护漂移→静默不播+京东出餐哑弹、zh-hk 双语种资产漂移、金额中文读法重写、设置开关两张皮、win7 播放失败队列永久卡死）+ 网络域指针；三例现行缺陷（正则 0.x0 全灭 / win7 catch 不回调 / retryCunt 不复位）按需求方裁全落并标「现行缺陷，接入前自查」
- **批2 措辞前向修正**（需求方本轮授权）：串口节尾注「客显侧共用串口工具」→「平行双实现、同坑同日双修」（importer 全集核实：serial_util.dart 11 个引用方全为客显，秤走独立文件）
- **指针面同步**：SKILL.md :23 索引行、agents/backend.md 设备分节列举补「数字客显 / 语音播报」（防 1.0.38 死指针闸同族）；KDS 节补叫号 TV 收银侧触发指针
- **验证**：test-self PASS=61 FAIL=0（含他会话 1.0.42/1.0.43 新增 5 闸与坑库症状速查对账闸）；T6 落盘新增面锚 100/100 cat-file 零 BAD（含硬非空守卫）；T2 占位全文件 0；T4 diff-check 绿；V5 脱敏全词（商户实名/门店号/订单号/broker 实例名/内部渠道词/人名/工单号）五落盘文件全零命中；V3 终验抽查 3 卡症状词→卡→锚定位成立
- **诚实边界**：①卡G 流控机制（RTS/DTR 未置位致不显示）为推断、无日志佐证，卡文如实标注；②语音播报宽口径 125 笔（方案 I3 初轮粗计）不可复现，实测口径 union 92 笔（no-merges）以复盘登记为准；③三例现行缺陷无历史事故锚，均为本轮代码审查+机械复验证实；④客显采证报告路径计数自报 33 实数 35（自报不可靠第 7 例，无卡受影响）；⑤saas-cashier stub `docs/platform-pitfalls.md`（9/14 停更）按 Q4 零触碰——处置建议：其仓 stub 顶部加一行「本档停更，外设坑库以 crules-flutter iot-devices.md 为准」或删除 stub 防误导检索
- **观测带数**：iot-devices.md 184→220 行（批1 起 58→220）；设备分节 8→10（+数字客显/语音播报）；批3 落盘卡 17（客显 8 + 播报 9）

## 1.0.43 · 升级三向合并批（方案甲）——`--force` 优先 base 合并、真冲突带标记呈人、`.new` 退人工

> 依据链：2026-09-23 两消费工程升级实测 `.new` 全人工痛（净改动 128/250 行、真人只需拍板各 1 冲突块）→ 方案 v1/v2 + plan-reviewer 隔离评审（docs/评审-plan-升级合并体验优化.md，#5 乙案 §七覆盖缺口否决、#6 三护栏、#9 mktemp）→ 需求方 09-24 裁「甲立批（带自存 base）+ 终态 D=指针引用」（docs/方案-2026-09-23-升级合并体验优化.md 台账 D-1~D-5）。本批=甲实施；D 迁移另批（三必测已全 PASS，前置门开）。与 1.0.42 parity 批同仓并行，test-self 按 hunk 拆分显式路径提交（双批协调协议）。

- **三态写入升级（install.sh `--force`）**：对 base **零改动直替** / 项目与模板**各自演进自动并**（写回+刷 base）/ **尾部撞戳位窄形自动解**（仅 CLAUDE.md：ours 段旧戳收尾+theirs 段仅 gen 围栏/新戳/空行，实跑两工程冲突同此形）/ **真冲突带标记写回**+`.crules-conflicts` 摘要呈人——`.crules-bak` 先备份、解标记前**戳与 base 禁更新**（护栏防 check-imports 假绿）；无 base 可比、merge 出错或 `--new-only`（1.0.42 及以前语义正门）退 `.new` 伴生
- **base 双来源（D-5）**：首装自存 `dst.crules-base` 快照（gitignore 三行伴生物随装落位）；历史工程首轮按项目戳退 cache 旧版模板（checklist/进阶 无自身戳，版本改取项目 CLAUDE.md 戳——真树负控抓出的缺口）
- **进阶六篇纳入合并**：`MERGE_REL` 从 CLAUDE.md+checklist 扩至进阶全家族——零改动模板件不再产 `.new` 噪音
- **叙事面**：README 升级节改写三向合并语义；init.md 分流段同步（评审 #8）
- **验证**：test-self PASS=61 FAIL=0（56 基线+合并四态/退路 5 fixture，mktemp；夹具含真冲突写回、窄形解戳唯一、`--new-only` 退路断言）；真树负控两消费工程沙盒副本（saas-suite/light 现状 v1.0.41 树）首轮 `--force` 各 8 文件全 MERGE、零冲突、零 `.new`、写回零残留标记，二轮幂等
- **诚实边界**：①历史工程首轮过渡依赖 cache 旧版目录存续（宿主保留策略不可控）——自存 base 才是正源，cache 仅为过渡；②`.new` 路径不写 base，未并入文件下轮仍退 `.new`；③窄形自动解只认可枚举形，同段语义重写一律呈人（宁冲突勿错并）；④`git merge-file` 只依赖 git 二进制，不要求目标/cache 是 git 仓，缺 git 退 `.new`
- **观测带数**：test-self 断言 56→61（+5）；常驻基线零变化（改动全在升级面安装器/闸面/docs）；消费工程升级体验从「逐行人工对照 7-8 伴生」缩至「拍板冲突块（实测 0-1 处）」

## 1.0.42 · parity-rewrite skill 批——复刻/重写/迁移既有模块三段法 + 零项目值双扫描与尺寸两闸（与 1.0.43 升级合并批同仓并行）

> 依据链：需求方会话指令（复刻/重写/迁移既有功能模块场景缺标准方法，2026-09-24）→ 本批直接落 skill + 闸，无独立方案文档（诚实边界见下）。与 flutter-4b 会话的升级三向合并批（1.0.43，f337c40）同仓并行，共享文件 scripts/test-self.sh 按 hunk 级拆分各自显式路径提交（双批协调协议：版本错开 + 共享面串行 + 让位信号）。

- **parity-rewrite skill（新，skill ×2）**：`skills/parity-rewrite/`——SKILL.md 三段法（对照清单钉住参照实现真实行为 → 逐项重建 → 测试矩阵验证一致）+ references 四篇（boundary-asset / orchestration / rework-ledger / closure）+ templates 对照清单模板一篇。触发：复刻、重写、重构、迁移、换栈、对齐某实现、以某版本为基准
- **test-self 两闸（54→56）**：①零项目值 + 引用悬空双扫描（扫 skills/parity-rewrite/ 全树：禁列 token 命中或 spec 节引用悬空即红，就地改写/换指针）②尺寸闸（SKILL.md ≤10000 字符、references/ 与 templates/ 单文件 ≤12000 字符；超限拆分下沉，不静默上调预算）
- **分发面登记**：README 场景表、help 场景表与 skill 计数行（skill ×2）各补 parity-rewrite
- **扫描豁免**：死链/黑话两扫描排除 `.superpowers/`（本机过程面：git 忽略、非分发面；此前撞的是协调文档内相对链接示例与正则字面，非仓内容器死链）
- **验证**：test-self PASS=61 FAIL=0（54 基线 + 本批 2 闸 + 1.0.43 合并四态/--new-only 5 fixture 共存态；两批各自提交前后复跑，绿为共存态实测非本批单侧）
- **诚实边界**：无独立方案文档，依据链即会话指令；references 四篇语义未经独立外审（闸只守项目值/悬空/尺寸三面）；1.0.42 版本缺口由缺口闸内建当前版本豁免（plugin.json discard，非登记行），tag 必达闸兜发版时序
- **观测带数**：test-self 断言 54→56（共存态 61）；常驻基线零变化（改动全在按需面 skill/闸面/docs）

## 1.0.41 · 轻量撞名消歧 + 版本缺口/欠账台账与三闸 + 行动甲 B/C 首采销账——外部第三轮评审 E1~E4 处置

> 依据链：2026-09-23 外部第三轮评审五条（E1 轻量两轴撞名 / E1b §数字死指针 / E2 采证单线四账同悬 / E3 校验者无失准信号 / E4 minor 通胀+计数未复核）→ 逐条实测核实后 plan-reviewer 隔离评审 v1（F-1~F-8：纠 E1b「三处」实为六+二、纠 E1 归因「1.0.38 加」实为批2b 引入 1.0.38 改写〔git log -L 实证〕、②白名单盖不住 :114 裸用、⑥债龄闸与 tag 必达闸结构冲突、负控 git tag -d 改假版本头注入）→ 需求方双裁决：「轻量 tier」本意＝任务轴 + 债龄闸「上闸即咬」。方案 `docs/方案-2026-09-23-轻量撞名消歧与单线采证解锁.md`。

- **撞名消歧（E1/G1）**：`进阶/方案评审闭环.md:106`「轻量 tier 不启用」改「收尾任务档之极简档（任务轴）不启用本篇完整闭环——不扩至标准档；项目档位不改变本篇启用条件」（F-3 纠扩围：只作等价消歧不夹带判据）；:114「轻量起见」→「简化起见」。**canonical/双模板生成面零触碰**（甲案：改后占用者）。死指针全量（E1b/F-1）：§3→§三 六处 + §2→§二 两处，工程化流程:76 篇内自引用合法保留
- **校验者陈述降级（E3/G4）**：校验层「有更优方案→需求方裁决」补「**校验者主张（非已验证事实）**，与 reviewer 推断级同等对待」；README 概念地图行同步。方案评审闭环启用条件节补记忆库关取材声明（只声明不加机制）
- **活台账 + 三闸（E4/E2/G3）**：新 `docs/清单-版本缺口与欠账.md`——11 版本缺口逐版核账分诊（**0 让位 / 2 历史不回补〔check-imports:36 既有政策预登〕/ 9 真实发版漏打〔1.0.11/1.0.26–33，需求方裁不回补、表即终态〕**——E4「漏打或让位混存」预判核后修正为「均非让位」）+ 欠账台账（债龄口径 ≤N=10）。test-self 51→**54**：①撞名闸（行级白名单「轻量〔light〕|轻量档」，注入「轻量 tier」负控红→还原绿留痕）②版本缺口闸（差集 ⊆ 登记表**双向对账**含幽灵行反查；负控=CHANGELOG 注入假版本头即红——不碰 git refs，F-6；非 git 树 SKIP）③欠账龄闸（plugin.json 派生当前版本数龄；**上闸即红系裁决「咬」**——封版通道压力即闸的存在理由）
- **行动甲 B/C 首采销账（E2 解锁，同批）**：消费工程 `saas-suite/…/saas-cashier`（戳 v1.0.40）——`/context` 任务前基线 56.8k/200k、Memory files 10.2k（宪法 9.9k+MEMORY 362）；**token↔字符首互证**：宪法实测 15,665 字符 ↔ 9.9k tok ≈ 0.632 tok/字，字符预算口径（app 16,571≈10.5k tok）同量级成立；`.compliance-log` 首行真实读数落盘（constitution_stale:false / 386 行 0 坏行 / truncated:false）——遵守度账本进「有数」（N=1，悬空态口径不变）。A 步（升级）09-23 实测已完成（双仓 v1.0.40，提醒闸共窗归因不写死）；D/E 继续在册
- **验证**：test-self **54/0**（三闸负控先红后绿留痕；⑤表销账行移「已销账」节、债龄闸只扫在册节）；plan-reviewer v1 隔离评审 F-1~F-8 全吸收；Windows 实机复跑=合入前置（§5 升格条款，待跑）
- **诚实边界**：①改后占用者措辞，已装消费工程旧文案升级自然收敛；「任务轴」读法系语义裁决非原文自证（批2b 原文「不复制」更贴部署语义），新句双向堵漏；E2「独苗」结构性含义仍在——B/C 首采系同仓系（saas-suite 宿主），第二项目异构（E）未达成不遮掩
- **观测带数**：①常驻基线零变化（改动全在按需面/闸面/docs；预算闸实跑绿 app 16571/plugin 15270）②distill 四数：样本 8/30 · 比率按规则不输出（悬空态显式，同 1.0.31–1.0.40 口径）③test-self 断言 51→54（+3 闸）

## 1.0.40 · 陈旧宪法提醒闸 + §一红线收 canonical + 锚串围栏化 + 幽灵版本引用清理——外部重核排序 N-1/N-2/N-5/N-6 处置

> 依据链：2026-09-23 外部重核六条（N-1 消费侧断层为咽喉 / N-2 新红线逐字双写无闸 / N-3 bus factor / N-4 /context 欠账 / N-5 幽灵 1.0.36 / N-6 verify 假绿族普查未做）→ 逐条实测：N-1 部分过期（本机插件今日已 1.0.39，剩**法治层**戳旧与"升级无人催"两点为真；.compliance-log 全宇宙零读数属实——本仓无 .claude/memory/ 定根即退）；N-4 记短了（欠账自 1.0.10 挂"下次 minor 实测"起非 1.0.32 起）；N-6 首行判错已纠（锚串守卫扫目标文件自身不扫 docs，病形系"同文件叙事句"非"叙事文档"，处方随之改道）。九轮追问的收敛点：**陈旧检测放 stop-reminder 队列判定之前**——设计评审抓出挂队列非空分支会对着"刚收尾、队列空"的目标宿主永不响（落点 bug，fixture 有回归钉）。

- **宪法陈旧提醒闸（N-1③）**：stop-reminder 定根后、读队列前独立比对 项目 CLAUDE.md 戳 vs 已装插件版（`_common` 新三件 plugin_version/constitution_stamp/version_key——compliance 私有 plugin_version 收编，跨 hook 符号绊线同步过闸），戳旧则 Stop 提醒「宪法版本落后：v旧→v新 + 可复制升级命令」；**队列空亦响**、stop_hook_active 续轮抑制、无戳/读不到静默（无从判不催）。执法点=每会话收尾必达位；升级动作不自动化（提醒≠代执行）。覆盖边界如实：轻量档（无 NAVIGATION）定根即退收不到本提醒——与队列/账本同覆盖面对齐。fixture +4（旧戳空队列响 / 续轮抑制 / 同版静默 / 无戳静默）
- **§一红线收 canonical 第 6/7 块（N-2）**：「引用皆有实测依据」「方案评审必核两件事」两条（实测两模板逐字双写、不邻接故两块）进 canonical，双模板套 gen 围栏；render EXPECTED 7 块、幂等复跑零写入。红线落地当日即成 canonical 未覆盖面——正是 1.0.39 防漂移批点名的病形复发，当日收口。**写死的块数随批去硬编码**（render 头注/README/测试自述三处「五块/18 条」系本批扩块当场自曝的下次漂移之源——「数字写死即漂移之源」同训第三现，交付前自查抓出非闸抓出，如实记）
- **锚串守卫围栏化（N-6 普查结案）**：锚串从「全文件 grep」改「须命中对应 gen 围栏区间内」——堵同文件叙事句假绿面（块被删、别处提到该短语旧法即瞎；负控双样本实证：掏空+叙事句样本旧法绿/新法红、空掏样本新法红）。普查四候选定性落账：锚串=中危已修 / verify-cache=已修（ac2c7a8）/ install 戳判定与 check-imports 读回=实物读非叙事 grep，结案不动。「缺口率不降因检测面每版在扩」的判语随本次结案记一笔。
- **幽灵 1.0.36 三处（N-5）**：compliance-audit.py 头注/AV 注/弱签名注的版本指认改实 1.0.37（跳号系队列让位、1.0.36 无实物——CHANGELOG 1.0.38 注在册）
- **验证**：test-self **52/0**（锚串 12 串全围栏化、孪生生成闸 7 块绿、收编双锁绿）；五 fixture 全绿（stop-reminder 18→22 含陈旧四断言；compliance 22→24 补 constitution_stale 两态——交付前自查揪出该账本列**零测试**、已补，与提醒闸同源）；render --check 幂等；deny 双驱动 + `--diff` 170 例两路同判 0；负控：锚串掏空样本红、叙事句样本旧绿新红、陈旧闸四态各如其分。AV 弹窗插曲：fixture 首跑 1 败为宿主注入崩溃（0xc000… 地址随机+复跑绿），非代码病，与 1.0.33 压制覆盖面（仅 hook 脚本体）一致的已知边界
- **诚实边界**：①陈旧提醒首圈生效前提=消费工程至少装过一版带戳宪法（saas-cashier 满足）；②账本 constitution_stale 与提醒闸同受定根约束（轻量档双缺，同源声明）；③N-1 的 Mac 侧插件 1.0.25 维持 9-20 快照未本机复验；④N-4 /context 实测债仍欠、N-3 bus factor 无机制解——行动甲（saas-cashier install 升级 + 首条真实读数 + /context 同车）待授权，非本批交付面
- **观测带数**：①常驻基线：双模板各 +4 行围栏注释（HTML 注释不入读者语义面；字符口径闸内绿）②distill 四数：消费工程样本 8/30 · 比率按规则不输出（悬空态显式，同 1.0.31–1.0.39 口径）
- 双 json 1.0.40 + README 横幅同步
