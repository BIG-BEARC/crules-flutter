...
> **当前状态：0.6.5 核验刷新**——两轮审查（时效性 S1-S16 + 外审 A-E）与 🟢 轻微项全数收口（0.6.0 适配三选一+字体策略+IoT 抽层 / 0.6.1 checklist 无障碍·性能节 / 0.6.2 轻微项清池+语义闸 / 0.6.3 性能·CI·发布·监控三空白补域+记忆库钩子推广+plugin §八实体化）；常驻基线 11.5K 已实测落档（B2 不触发）；test-self 15 断言全绿；余 POS 垂直拆层一项（开源定位裁决）待方案；0.6.4 吸收 saas-cashier 生产实证四笔（列表 key 错乱 / SP 白屏 / 图片解码 / Impeller Vulkan）。
