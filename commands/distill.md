...
- **review 裁决聚合**（`.claude/memory/.review-ledger` 存在才执行；**不存在或有效行为零 → 不静默跳过**，汇报输出显式异常行「距上次 distill 零新增 ledger 行——度量空转信号（止损线观察点：2.0 前仍零样本则砍四数、只留 ledger 原始账）」）：读全量 JSONL——字段 `date / agent / fid / sev / level / verified / verdict / note`（verdict ∈ 采纳 · 改写采纳 · 误报 · 撤条 · 搁置；`fid` 仅单报告内唯一、跨会话不唯一——勿按 fid 追踪裁决变迁；`note` 禁英文引号与换行——引号坏 JSON 行，中文引号「」表述）；**缺字段或坏行一律跳过并计数报告**，`搁置` 不计任何分母。**N < 30 只列明细不输出比率**（防小样本 Goodhart 化）；N ≥ 30 输出四数——总发现数 / 采纳率（采纳+改写采纳）/ 误报率（误报+撤条）/ **按验证状态分桶的误报率**（`verified=推断` 被证伪比例——指导 reviewer.md 审查纪律修订的真信号）。异常模式（如推断级误报率 >50%）作提名进 §5 闸；聚合输出进汇报，**不常驻任何文件**
