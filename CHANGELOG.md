...
## 1.0.26 · 裁决必附具体内容——红线路由条款 + §五 裁决区固定槽

> 依据链：需求方反馈（2026-09-20）——裁决请求常不附条目内容，需求方需上翻找裁决对象。五处裁决点盘点：`AskUserQuestion` 选项卡（多方案 / distill 闸）天然就近；review 逐条裁决（§五）、校验层列单裁决（收尾三档）、更简方案推回（§三）为散文呈现，此前无任何条款强制附条目。

- **app + plugin 双模板 §一 协作红线各 +1 条「裁决必附具体内容」**（「多方案走选项卡」之后）：路由式就近——可枚举裁决走 `AskUserQuestion`（条目全文进问题与选项说明，超 4 条分批、每批自包含——宿主渲染在交互点，物理就近）；不进选项卡的（长列单 / 开放裁决）就地列条目全文（编号 + 位置 + 原文 + 建议 + 上下文）；禁止只写「请裁决」「等确认」不附内容、或回指很早之前的消息。**诚实边界**：纯提示层条款，无机器闸可拦散文输出（「请求裁决」发生在自由文本，无工具调用面），强制力靠红线位常驻
- **双模板 §五 交付汇报定义补「裁决区」固定槽**：收口类裁决钉在报告末尾——有待裁决条目时逐条全文内嵌（编号 + 位置 + 原文 + 建议），无则写明「无待裁决项」；极简档一行汇报豁免。与红线条款合成闭环：中途裁决走选项卡（宿主渲染物理就近），收口裁决看报告尾（位置可预测、槽缺即违规一眼可核对）
- **观测带数**：常驻基线——app 16429/18000、plugin 15128/16500 字符（红线条款 +约 190、裁决区槽 +约 85，预算闸内）；distill 四数——仍无数（止损线 2026-12-31）
- **验证**：test-self 28/0 两轮实跑绿（本机 Windows；§一 落条后一轮、§五 落槽后一轮；常驻面预算闸 + 双模板孪生结构闸覆盖本改动）；grep 两模板「裁决必附具体内容」「裁决区」各 1 处；工作区同期的范围外变更（docs/方案-2026-09-14-review结果校验机制.md，P1 试点采证）已由另一会话以纯 docs 批先行提交（cd9052a，未占版本号，无撞号），不入本批
