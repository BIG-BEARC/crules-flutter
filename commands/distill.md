...
- **review 裁决聚合**（`.claude/memory/.review-ledger` 存在才执行）：读全量 JSONL——字段 `date / agent / fid / sev / level / verified / verdict / note`（verdict ∈ 采纳 · 改写采纳 · 误报 · 撤条 · 搁置）；缺字段行跳过并计数报告，`搁置` 不计任何分母。**N < 30 只列明细不输出比率**（防小样本 Goodhart 化）；N ≥ 30 输出四数——总发现数 / 采纳率（采纳+改写采纳）/ 误报率（误报+撤条）/ **按验证状态分桶的误报率**（`verified=推断` 被证伪比例——指导 reviewer.md 审查纪律修订的真信号）。异常模式（如推断级误报率 >50%）作提名进 §5 闸；聚合输出进汇报，**不常驻任何文件**
