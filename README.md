...
> **当前状态：1.0.5 二轮评审三发现收口**——gitignore 全程机制化（install.sh 幂等落位三行，取代 1.0.4 模板侧文字指引）+ §五 细则下沉 + docs 轮次化义务成文；1.0.4 ledger 外审收口（note 转义 + fid 语义）；1.0.3 审查质量落账（裁决 ledger + 四数聚合）；1.0.2 评审后优化批一；1.0.1 docs 清账（−93%）；1.0.0 独立成库（deny-list Vendor 终态）；常驻基线 14.4K（距 15K 线 0.6K，归因口径见维护节）。
