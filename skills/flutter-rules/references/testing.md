...
> **表征断言先实测再落锁（2026-09-20 增，两表通用）**：锁既有行为 / 几何 / 视觉的表征断言，断言值取**实测可达值**——先跑一次拿数、查框架机制给界，不取理想值；理想值框架结构性给不了时测试恒红进仓（实证：InputDecorator 基线制定位下 hint 上下间隙差 ≤0.5 结构性不可达，红断言随批进仓后返工重写）。锁下的实测值注明机制下限与出处；残差视觉上可否接受属需求方裁决项，不当默认结论。
