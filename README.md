...
> **当前状态：0.6.4 实证吸收**——两轮审查（时效性 S1-S16 + 外审 A-E）与 🟢 轻微项全数收口（0.6.0 适配三选一+字体策略+IoT 抽层 / 0.6.1 checklist 无障碍·性能节 / 0.6.2 轻微项清池+语义闸 / 0.6.3 性能·CI·发布·监控三空白补域+记忆库钩子推广+plugin §八实体化）；常驻基线 11.5K 已实测落档（B2 不触发）；test-self 15 断言全绿；余 POS 垂直拆层一项（开源定位裁决）待方案；0.6.4 吸收 saas-cashier 生产实证四笔（列表 key 错乱 / SP 白屏 / 图片解码 / Impeller Vulkan）。
...
- **常驻基线实测**（0.6.1 后落档，2026-09-08，真实消费工程 saas-suite `/context` 快照）：Memory files **11.5k**（CLAUDE.md 11.1k + memory 索引 325）/ Skills **8.1k**（薄索引形态）/ plugin agents 前置体合计 **151**——**低于设计方案 §4.8 轻量模式触发线 15K**，常驻瘦身（外审 B2）**不触发**，后续每次 minor 复测一次，越线再单开
