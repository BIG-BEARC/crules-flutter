...
**8. Git 规范**：commit message 清晰；不提交不该入库的生成文件；不提交调试临时代码；iOS 生成文件（Generated.xcconfig / flutter_export_environment.sh）按机器路径假变更，pathspec 排除后再提交；多仓 / 子模块 git 一律 `git -C <绝对路径>`，push 等外发操作前命令内先 `git remote -v` 核对目标仓再推（实战 7 踩：cwd 在命令间重置，裸 push 把主仓推出了新远端分支）。
...
> **0.6.0 增补差集标注**（订单折算复盘吸收，R1-R4）：金额展示铁律→并入 9 ｜ 窄屏极值→并入 4 ｜ iOS 生成文件 pathspec→并入 8 ｜ 折算自查→新增「复刻 / 折算专项」条件条。
...
- [ ] 复刻在线版接口消费时追到 **logic / 消费端**确认字段是服务端返回还是**前端加工**（entity 里有字段 ≠ 接口会返回——实证：名称是消费端按 `payType` 前端补写的，模型猜 `payName`/`paymentName` 皆空）；entity 头部**样例响应注释优先看**，可省一轮返工
